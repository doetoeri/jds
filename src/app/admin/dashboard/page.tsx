...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchAllStats = async () => {
        setIsLoading(true);
        try {
            // Basic Stats (will be updated by listeners, but fetched once for initial load)
            ...
            setStats(prev => ({...prev, openInquiries: inquiriesSnapshot.size }));

            const productsQuery = query(collection(db, 'products'));
            const productsSnapshot = await getDocs(productsQuery);
            setStats(prev => ({...prev, totalProducts: productsSnapshot.size }));

            // Complex stats (calculated once)
            let totalLak = 0;
            let totalCredits = 0;
            let totalDebits = 0;
            const studentUsers = usersSnapshot.docs.filter(doc => doc.data().role === 'student' || doc.data().role === 'teacher');
            ...
            const studentDocsWithId = usersSnapshot.docs.filter(doc => doc.data().role === 'student' && doc.data().studentId);
            for (const userDoc of studentDocsWithId) {
                const studentId = userDoc.data().studentId;
                const grade = studentId.substring(0, 1);
                if (grade === '1' || grade === '2' || grade === '3') {
                    const gradeKey = `${grade}학년`;
                    const userTransactionsSnapshot = await getDocs(collection(userDoc.ref, 'transactions'));
                    userTransactionsSnapshot.forEach(transDoc => {
